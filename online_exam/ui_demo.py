...
# --- KHỞI TẠO DỮ LIỆU ẢO ---
@st.cache_resource
def setup_dummy_data(teacher_id):
    room = service.create_class(teacher_id, f"Lớp Demo {datetime.now().strftime('%H%M%S')}")
    st_spam, _ = service.create_student(teacher_id, room.id, "Học sinh Spam", f"hs_spam_{int(time.time())}")
    ...
    return room.id, st_spam, exam_id, exam_for_hs, prob1, res_correct

@st.cache_resource
def setup_50_students(teacher_id, class_id, exam_id, prob1_id):
    with st.spinner("Đang tạo ảo 50 học sinh (chỉ chạy 1 lần duy nhất)..."):
        students = []
        for i in range(50):
            ...
            pr = service.get_or_create_problem_progress(en.id, prob1_id)
            students.append((st_i, en, pr))
        return students

def page_demo(teacher_id: int):
    ...
    try:
        class_id, st_spam, exam_id, exam_for_hs, prob1, res_correct = setup_dummy_data(teacher_id)
    ...
    if st.button("🚀 Kích hoạt Tấn công Spam (10 Requests)", type="primary"):
        ...
        def spam_task(i):
            time.sleep(0.1) # Đồng bộ các luồng
            try:
    ...
    if st.button("🔥 Kích hoạt 50 Học sinh nộp đồng loạt"):
        students_data = setup_50_students(teacher_id, class_id, exam_id, prob1["id"])
